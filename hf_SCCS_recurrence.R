...
df2 <- cohort %>% 
  ...
  left_join(dx_after, "patient_pssn") %>% 
  mutate(eventdate = date) %>% 
...
df3 <- df2 %>% 
  ...
  # filter(sex == "M") %>%
  # filter(sex == "F") %>%
  # filter(hx.covid == 0) %>% # sen3 eTable5
  mutate(nid = as.integer(patient_pssn)) %>%
...
temp <- df3 %>% group_by(nid) %>% summarize(count = n()) %>% mutate(ngrp = if_else(count>=2, 2, 1))
df3 <- df3 %>% mutate(ngrp = temp$ngrp[match(nid, df3$nid)])
...
(eventde_result <- standardsccs(event ~ factor(ngrp)/vaccdate1+factor(ngrp)/vaccdate1_p14+ factor(ngrp)/vaccdate2+ factor(ngrp)/vaccdate2_p14+ factor(ngrp)/vaccdate3+ factor(ngrp)/vaccdate3_p14+age,
                             indiv = nid,
                             astart = obs_start,
                             aend = obs_end,
                             aevent = eventdate,
...
str_c(round(eventde_result$conf.int[seq(15,26),], 2)[,1], "(",
      round(eventde_result$conf.int[seq(15,26),], 2)[,3], "-",
      round(eventde_result$conf.int[seq(15,26),], 2)[,4], ")")
