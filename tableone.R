...
final_cohort <- read_rds("HF result/final_cohort.rds") %>% select(patient_pssn, vaccine.brand.1st, Age, sex, death_date_ymd, death_diag_cd) %>% 
...
dx_codes <- setDT(read_excel("HF result/codes.xlsx", sheet="codes"))[!is.na(Name)]
...
drug_codes <- setDT(read_excel("HF result/codes.xlsx", sheet="drugs"))[!is.na(Name)]
...
final_cohort[,c(seq(8, 40))] <- lapply(final_cohort[,c(seq(8, 40))], factor)
...
write.csv(print(tableone, quote = F, noSpaces = F), file = "HF result/tableone.csv")
